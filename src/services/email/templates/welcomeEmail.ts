...
export function renderWelcomeEmail(input: WelcomeEmailInput): EmailRenderResult {
  ...
  const subject = "Welcome to LifePack";
  const text = [
    `Welcome, ${cleanDisplayName(input.name)}`,
    "",
    "Your LifePack is ready.",
    "",
    ...
    "",
    "This message was sent by LifePack.",
  ].join("\n");
  ...
  const html = renderShell(`
    ...
    <tr>
      <td style="font-family:'Space Grotesk',Inter,Arial,Helvetica,sans-serif;font-size:22px;line-height:1.25;font-weight:700;color:${EMAIL_STYLE.clay};padding:0 0 14px 0;">Your LifePack is ready.</td>
    </tr>
    <tr>
    ...
    <tr>
      <td style="border:1px solid ${EMAIL_STYLE.borderDeep};border-radius:16px;background:rgba(255,255,255,.72);padding:16px 18px;font-size:14px;line-height:1.65;color:${EMAIL_STYLE.body};">Private by design. LifePack will never ask for your password by email.</td>
    </tr>
    <tr>
      <td style="padding-top:24px;font-family:'JetBrains Mono','Courier New',monospace;font-size:11px;line-height:1.6;letter-spacing:1.4px;text-transform:uppercase;color:${EMAIL_STYLE.muted};">Account message - LifePack</td>
    </tr>`);
