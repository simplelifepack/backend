...
export function renderWealthHandoffEmail(input: {
  ...
  const safeOwner = input.ownerName?.trim() || "A LifePack member";
  const security = "The attached ZIP contains only the intended shared Wealth documents and summary. LifePack never includes passwords, encryption keys, authentication tokens, or connected account credentials.";
  return {
    subject: `LifePack Wealth SOS Handoff from ${safeOwner}`,
