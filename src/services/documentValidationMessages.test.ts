...
test('upload errors are distinct and safe in development and production', () => {
  for (const production of [true, false]) {
    for (const [code, message] of [
      ...
      ['FILE_TOO_LARGE', 'File is too large. Please upload a file no larger than 10 MB.'],
    ]) {
...
test('invalid metadata size and unsupported format do not become encryption or corruption errors', () => {
  ...
  assert.throws(() => validateEncryptedDocumentEnvelope({ originalSize: 10 * 1024 * 1024 + 1 }, file), { code: 'FILE_TOO_LARGE', statusCode: 413 });
